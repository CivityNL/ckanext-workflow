...
from ckanext.workflow.model.workflow_request_message import WorkflowRequestMessage, define_workflow_request_message_table

log = getLogger(__name__)

workflow_state_table = None
workflow_request_table = None
workflow_request_message_table = None


def setup():
    global workflow_state_table
    global workflow_request_table
    global workflow_request_message_table
    ...
    if not all([workflow_state_table.exists(), user_table.exists()]):
    ...
    if workflow_request_message_table is None:
        workflow_request_message_table = define_workflow_request_message_table()
        log.debug('Workflow request message table defined in memory')

    # workflow_request_message_table depends on workflow_request_table
    if not all([workflow_request_table.exists(), user_table.exists()]):
        log.debug('Workflow request message table creation deferred')
    else:        
        if not workflow_request_message_table.exists():
            workflow_request_message_table.create()
            log.debug('Workflow request message table created')
        log.debug('Workflow request message table already exists')
