...
def package_update(original_action, context, data_dict=None):
    ...
    print(f"chained_auth_function package_update result={result}")
    return result
...
def package_set_state(context, data_dict):
    ...
    print(f"auth package_set_state allowed_states={allowed_states} state={state}")
    if data_dict.get("state") not in allowed_states:
        result = {"success": False}
    print(f"auth package_set_state result={result}")
...
def workflow_chained_auth_function(action, getter):
    ...
    def chained_auth_action(original_action, context, data_dict):
        ...
        if not original_success:
            print(f"chained_auth_action {action} original_success = {original_success}")
            result = original_result
